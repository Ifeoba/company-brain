...
description: Validate whether the user has enough material and encoded decisions to actually have a "company brain" for a specific service \'97 as opposed to a knowledge base, a doc dump, or a chatbot over PDFs. Use this skill whenever the user wants to check company brain readiness, audit their materials for a domain, validate a capstone submission against the company-brain framework, assess OPT framework completeness, verify they have encoded judgment and skills not just retrieval, or asks anything like "do I have enough?", "is this a brain yet?", "is my company brain ready?", or "can I build an agent on top of what I have?" Runs in two modes \'97 an interactive interview walking through validation questions one dimension at a time, and an audit mode that ingests user-supplied materials (docs, notes, transcripts, exports) and produces a readiness report across six dimensions covering service definition, knowledge extraction, judgment encoding, skills, guardrails, and proof of real work.\
---\
\
# Company Brain Validator\
\
A skill to test whether the user actually has a **company brain** \'97 an executable, decision-making operating map of how a specific service runs at a specific company \'97 or whether they have a knowledge base, a doc dump, or a chatbot over PDFs.\
\
...
\
1. **Interview mode** \'97 Walk the user through validation questions, one dimension at a time. Use when they have nothing concrete to upload yet, or want to think out loud.\
2. **Audit mode** \'97 Ingest their materials (docs, notes, transcripts, screenshots, exports, links) and assess them against the rubric. Use when they have artifacts to evaluate.\
\
...
\
Validation is structured around six dimensions. The full rubric \'97 with missing / partial / solid levels and concrete examples \'97 lives in `references/rubric.md`. Read that file before scoring any dimension; do not score from memory.\
\
1. **Service Definition** \'97 Is the unit of work narrow, real, and paid for today?\
2. **Knowledge Layer** \'97 Has the messy operating knowledge been extracted from where it actually lives (interviews, Slack, email, spreadsheets, ticket history), or only from the official SOP?\
3. **Judgment Layer** \'97 Are decision rules, exceptions, and "what good looks like" encoded? Is there an eval set with known correct outcomes, including hard cases?\
4. **Skills / Action Layer** \'97 Are there discrete skills with clear inputs/outputs that connect to real systems? Can the agent execute, or only describe?\
5. **Guardrails** \'97 Is the line between "agent decides", "human decides", and "never do this" encoded in the brain itself, not just in the user's head?\
6. **Proof** \'97 Has the agent done a real unit of work that a human verified?\
\
...
4. After each dimension, give a one-line summary of what you heard, a tentative status (\uc0\u10060  Missing / \u9888 \u65039  Partial / \u9989  Solid), and ask if it sounds right before moving on.\
\
...
2. Read the materials. For documents, read them carefully \'97 don't skim. For long materials, identify which sections speak to which dimension.\
...
\
**The standard the user is being measured against is \uc0\u9989  Solid on every dimension as defined in `references/rubric.md`.** That is the bar a reviewer or operator would defend as a real company brain. Anything below \u9989  is a gap to close, and the report must spell out exactly what work closes it.\
\
```\
...
\
## Verdict\
[One of: "Not yet a brain \'97 concept stage." / "Knowledge base, not a brain." / "Brain in formation \'97 gaps remain." / "Brain ready \'97 agent can execute on top."]\
\
...
|---|---|---|---|\
| 1. Service Definition | \uc0\u9989  / \u9888 \u65039  / \u10060  | ... | ... |\
| 2. Knowledge Layer | \uc0\u9989  / \u9888 \u65039  / \u10060  | ... | ... |\
| 3. Judgment Layer | \uc0\u9989  / \u9888 \u65039  / \u10060  | ... | ... |\
| 4. Skills / Action Layer | \uc0\u9989  / \u9888 \u65039  / \u10060  | ... | ... |\
| 5. Guardrails | \uc0\u9989  / \u9888 \u65039  / \u10060  | ... | ... |\
| 6. Proof | \uc0\u9989  / \u9888 \u65039  / \u10060  | ... | ... |\
\
(\uc0\u9989  Solid, \u9888 \u65039  Partial, \u10060  Missing)\
\
## Work needed to reach the standard\
\
For every dimension scored below \uc0\u9989  Solid, include a block in this exact format. Skip dimensions already at \u9989 . Pull the "target state" language from the \u9989  Solid level in `references/rubric.md`.\
\
### [Dimension N: Name] \'97 current \uc0\u9888 \u65039  Partial / \u10060  Missing \u8594  target \u9989  Solid\
\
...
\
**Target state**: [What \uc0\u9989  Solid looks like for this dimension, in one or two sentences from the rubric.]\
\
...
\
**Evidence the gap is closed**: [An inspectable artifact, test result, or sign-off that would let the user confirm this dimension is now \uc0\u9989  Solid.]\
\
## Priority order\
\
Across all gaps above, the order in which to tackle them. Stop at the number that's actually high-leverage \'97 usually 3, sometimes fewer.\
\
...
[One paragraph naming the single most likely way this becomes a chatbot-over-docs instead of a brain. Be specific to the user's situation \'97 generic warnings are noise.]\
```\
\
...
**Better**: "Interview the AR analyst (Priya) for 45 minutes. Capture which vendors get net-30 vs net-60, which exceptions she handles without writing them down, and who approves payments over \uc0\u8377 50k. Write this up as `exceptions.md` in the brain."\
\
...
**Better**: "Write `guardrails.md` with three explicit categories: (a) what the agent decides alone (e.g., invoices under \uc0\u8377 10k that match a PO), (b) what it escalates and to whom in what format (e.g., mismatches over \u8377 50k \u8594  Slack DM to founder with PO and invoice attached), (c) what it never does (e.g., never modify a PO, never send a payment instruction without human approval). For each, name the check in the brain that enforces it."\
\
...
\
Do not soften \uc0\u10060 s with hedging language. "Knowledge layer is partial \'97 you have the SOP but no interview evidence" is more useful than "Great start! There's room to grow on the knowledge side."}